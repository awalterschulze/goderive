...
func (this *gen) genMap(typ *types.Map) error {
	...
	// minFunc := this.min.GetFuncName(intSlice, types.Typ[types.Int])
	p.P("intersect := make(map[%s]struct{})", typeStr)
...
func (this *gen) genSlice(typ *types.Slice) error {
	...
	p.In()
	// intSlice := types.NewSlice(types.Typ[types.Int])
	// minFunc := this.min.GetFuncName(intSlice)
	p.P("intersect := []%s{}", typeStr)
